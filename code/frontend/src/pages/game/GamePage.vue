    ...
    <div class="w-full max-w-md space-y-8">
      <div class="text-center">
        ...
        <p class="mt-2 text-sm text-gray-600">
          You are playing <strong>{{ mode }}</strong> with <strong>{{ variant }}</strong> variant.
        </p>
        <p v-if="isMultiplayer && roomCode" class="mt-1 text-xs font-semibold text-blue-600">
          Room: {{ roomCode }}
        </p>
      </div>
      ...
      <div
        v-if="!isMultiplayer && !isGameActive"
        class="mb-4 p-4 bg-yellow-100 border border-yellow-400 rounded-lg text-center"
      >
        <p class="text-yellow-800 font-semibold mb-2">
          Game state was lost. Please start a new game!
        </p>
      ...
      <div
        v-if="!isMultiplayer"
        class="flex gap-8 mb-8 text-lg font-semibold justify-center"
      >
        <div class="text-center">
      ...
      <!-- BOTÃO DEBUG -->
      <div v-if="!isMultiplayer" class="flex justify-center mb-4">
        <Button
          @click="debugForceWin"
          class="py-1 px-3 bg-purple-500 text-white text-xs rounded hover:bg-purple-600"
        >
          DEBUG: Force Win
        </Button>
      </div>
      ...
      <div
        v-if="!isMultiplayer && game.winner"
        class="mb-4 text-2xl font-bold text-green-600 text-center"
      >
      ...
      <div
        v-else-if="!isMultiplayer && isGameActive"
      ...
      <div
        v-else-if="!isMultiplayer && game.currentTurn"
        class="mb-4 text-lg font-semibold text-center"
      >
      ...
      <div
        v-if="isMultiplayer"
        class="mb-4 p-4 bg-blue-100 border border-blue-400 rounded-lg text-center"
      >
        <p class="text-blue-800 font-semibold">Multiplayer game loading...</p>
        <p class="text-blue-600 text-sm mt-2">
          This should use the MultiplayerGame component
        </p>
      </div>
    </div>
  </div>
...
onMounted(async () => {
  if (!authStore.user) {
    try {
      ...
      console.error('Failed to load auth user on game page', e.response?.data || e.message)
    }
  }
...
const isGameActive = computed(() => {
  return (
    game.player1.length > 0 ||
    game.player2.length > 0 ||
    game.remainingDeck.length > 0
  )
})

// --- NOVO: guardar jogo quando termina (singleplayer) ---
async function finishGame() {
  if (isMultiplayer.value || !game.winner) return

  const humanId = authStore.user?.id
  if (!humanId) return

  const humanPoints = game.player1Points
  const botPoints = game.player2Points
  const humanWon = game.winner === 1

  console.log('Saving game...', { humanPoints, botPoints, humanWon })

  try {
    await apiStore.createGame({
      type: variant,              // '3' ou '9'
      status: 'Ended',
      player1_points: humanPoints,
      player2_points: botPoints,
      winner_user_id: humanWon ? humanId : null,
    })
...
async function debugForceWin() {
  // só singleplayer
  if (isMultiplayer.value) return

  // força pontos e winner
  game.player1Points = 120
  game.player2Points = 0
  game.winner = 1

  console.log('DEBUG: forced win, calling finishGame')
  await finishGame()
}

// Quando o winner passa de null -> 1/2, o jogo acabou
  watch(
    () => game.winner,
    (newVal, oldVal) => {
      console.log('watch winner', oldVal, '=>', newVal)
      if (!oldVal && newVal) {
        finishGame()
      }
    }
